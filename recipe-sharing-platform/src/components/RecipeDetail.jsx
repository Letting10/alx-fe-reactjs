...
function RecipeDetail() {
  ...
  if (!recipe) {
    return <div className="text-center mt-20 text-red-500 text-xl font-semibold">Recipe not found.</div>;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      ...
      <div className="bg-gradient-to-b from-yellow-50 to-yellow-100 rounded-3xl shadow-2xl overflow-hidden transition-transform transform hover:scale-102 duration-300">
        <img src={recipe.image} alt={recipe.title} className="w-full h-80 object-cover rounded-t-3xl" />

        <div className="p-8 pl-12">
          <h1 className="text-4xl font-extrabold mb-4 text-yellow-800">{recipe.title}</h1>
          ...
          <div className="mb-6">
            ...
            <ul className="list-disc list-inside text-gray-800 space-y-1 mt-2 pl-4">
              {recipe.ingredients?.map((ingredient, idx) => (
                <li key={idx} className="hover:text-green-600 transition-colors duration-200">{ingredient}</li>
              ))}
            </ul>
          </div>
